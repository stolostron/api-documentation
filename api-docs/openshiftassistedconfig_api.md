...
| └>&nbsp;&nbsp; **name** | `string` | Name specifies an environment variable reference (e.g., "$METADATA_HOSTNAME") from which to read the node name. The environment variable must be available in /etc/metadata_env (populated by configdrive). The value will be resolved safely and used to set the hostname. | N/A |
...
|  **infraEnvRef** | `object` | InfraEnvRef references the infra env to generate the ISO | N/A |
