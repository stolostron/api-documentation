...
|  **machineNetwork** | `string` | MachineNetwork is the subnet provided by user for the ocp cluster. This will be used to create the node network and choose ip address for the node. Equivalent to install-config.yaml's machineNetwork. Deprecated: Use MachineNetworks instead. | N/A |
|  **machineNetworks** | `array` | MachineNetworks is the list of IP address pools for machines. This enables dual-stack support by allowing multiple networks. If both MachineNetwork and MachineNetworks are specified, MachineNetwork should match the first element of MachineNetworks. Use instead of MachineNetwork. | N/A |
