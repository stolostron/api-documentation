...
| └>&nbsp;&nbsp; **upgradeType** | `string` | UpgradeType specifies which components to upgrade for HostedCluster deployments. For standalone clusters, this field is ignored. Supported values are ControlPlane (upgrade only the HostedCluster control plane), NodePools (upgrade only the NodePools/worker nodes), or empty string (default, upgrade both control plane and node pools). | N/A |
| └>&nbsp;&nbsp; **nodePoolNames** | `array` | NodePoolNames specifies which NodePools to upgrade when upgradeType is NodePools or empty (default). If not specified or empty, all NodePools associated with the HostedCluster will be upgraded. This field is ignored when upgradeType is ControlPlane. | N/A |
