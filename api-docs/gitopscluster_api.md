...
| └>&nbsp;&nbsp; **gitOpsOperatorImage** | `string` | GitOpsOperatorImage specifies the GitOps operator container image. Default is empty. This is the only image that needs to be specified - the operator uses environment variables to determine all other component images (ArgoCD, Redis, etc.). | N/A |
